...
class _FacebookFeedsState extends State<FacebookFeeds> {
  ...
  List<int> savedFacebookPost = [0, 2, 7];
    ...
    return Row(
      ...
      children: <Widget>[
        ...
        Row(
          children: <Widget>[
            IconButton(
                icon: Icon(Icons.favorite, color: (savedFacebookPost.contains(position)
                    ? (Colors.red)
                    : (Colors.grey.shade600)), size: 25),
                onPressed: () {
                  if (savedFacebookPost.contains(position)) {
                    savedFacebookPost.remove(position);
                  } else {
                    savedFacebookPost.add(position);
                  }
                  setState(() {});
                }),
            Transform.translate(
  ...
  Widget _text() {
    return Container(
      ...
      child:Text('We also talk about the future of work as the robots',style: TextStyle(fontSize: 15),),
    );
  }
  ...
  Widget _comment() {
    return Wrap(
      ...
      children: <Widget>[
        ...
        FlatButton(onPressed: (){}, child: Text('#facebook',style: TextStyle(color: Colors.orange),)),
      ],
    );
